...
def SCMP_CMP(arg=0, comparison=SCMP_CMP_EQ, arg1=0, arg2=0):
    ...
    #     assert comparison in [SCMP_CMP_EQ, SCMP_CMP_GE, SCMP_CMP_GT, SCMP_CMP_LE, SCMP_CMP_LT, SCMP_CMP_MASKED_EQ,
    #                           SCMP_CMP_NE]
    # except:
...
class Arg(object):
    def __init__(self, nr):
        try:
            ...
            raise AttributeError("nr must integer value.")

    def _get_rule(self, value, op):
        try:
            value = int(value)
        except:
            raise AttributeError('only integer values are supported.')
        return SCMP_CMP(comparison=op, arg=self.nr, arg1=value)

    def __eq__(self, other):
        return self._get_rule(other, SCMP_CMP_EQ)

    def __le__(self, other):
        return self._get_rule(other, SCMP_CMP_LE)

    def __ge__(self, other):
        return self._get_rule(other, SCMP_CMP_GE)

    def __lt__(self, other):
        return self._get_rule(other, SCMP_CMP_LT)

    def __gt__(self, other):
        return self._get_rule(other, SCMP_CMP_GT)

    def __ne__(self, other):
        return self._get_rule(other, SCMP_CMP_NE)

    def masked_arg_equal(self, value, mask):
        try:
            value = int(value)
            mask = int(mask)
        except:
            raise AttributeError('only integer values are supported.')
        return SCMP_CMP(comparison=SCMP_CMP_MASKED_EQ, arg=self.nr, arg1=mask, arg2=value)
...
def main():
    ...
    s = _ffi.new('char[256]', '../unsafe_program')
    print type(_ffi.string(s, 256))
    print(
    seccomp_rule_add(filter, ALLOW, syscalls.EXECVE, 1, SCMP_CMP(comparison=SCMP_CMP_EQ, arg1=_ffi.cast('scmp_datum_t', s))))


    ret = seccomp_load(filter)
    seccomp_release(filter)
    os.execve(_ffi.string(s, 256), ['./unsafe_program', 'asd'], {})
    os._exit(0)


# if __name__ == '__main__':
#     sc = SecureComputing()
#     sc.allows_rule(syscalls.WRITE, Arg(0) == sys.stderr.fileno())
#     sc.load()
#
#     sys.stderr.write('blad\n.')
